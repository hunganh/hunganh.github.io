...
function showEvaluationResult() {
    ...
    if (basicEvaluationValue === 0 && dcfEvaluationValue === 0) {
        ...
        $("#stockEvaluationResult").text(Intl.NumberFormat(numberLocale).format(evaluationValue.toFixed(0)));
        $("#showLoadingEvaluation").html("");
    }
}
...
function drawBasicEvaluationIndex() {
    ...
    $("#evaluation-pe").text(`${data.pe !== null ? Intl.NumberFormat(numberLocale).format(data.pe) : "N/A"}`);
    $("#evaluation-pb").text(`${data.pb !== null ? Intl.NumberFormat(numberLocale).format(data.pb) : "N/A"}`);
    $("#evaluation-eps").text(`${data.eps !== null ? Intl.NumberFormat(numberLocale).format(data.eps) : "N/A"}`);
    $("#evaluation-bvps").text(`${data.bvps !== null ? Intl.NumberFormat(numberLocale).format(data.bvps) : "N/A"}`);
    $("#evaluation-ebitda").text(`${data.ebitda !== null ? Intl.NumberFormat(numberLocale).format(data.ebitda) : "N/A"}`);
    var res = "";
        ...
        if (data[type] !== null) {
            data[type].forEach(item => {
                res += `<tr>
                            ...
                            <td class="text-right"><span class="bold-text dashed-border-bottom">${item.pe !== null && item.pe > 0 ? Intl.NumberFormat(numberLocale).format(item.pe) : "N/A"}</td>
                            <td class="text-right"><span class="bold-text dashed-border-bottom">${item.pb !== null && item.pb > 0? Intl.NumberFormat(numberLocale).format(item.pb) : "N/A"}</td>
                            <td class="text-right"><span class="bold-text dashed-border-bottom">${item.evebitda !== null  && item.evebitda > 0 ? Intl.NumberFormat(numberLocale).format(item.evebitda) : "N/A"}</td>
                        </tr>`;
            });
        } else {
    ...
    res += ` <tr class="sum-value-area">
                ...
                <td class="text-right"><span class="bold-text dashed-border-bottom text-white">${data.coefficient[type].pe !== null && data.coefficient[type].pe > 0 ? Intl.NumberFormat(numberLocale).format(data.coefficient[type].pe.toFixed(0)) : "N/A"}</td>
                <td class="text-right"><span class="bold-text dashed-border-bottom text-white">${data.coefficient[type].pb !== null && data.coefficient[type].pb  > 0 ? Intl.NumberFormat(numberLocale).format(data.coefficient[type].pb.toFixed(0)) : "N/A"}</td>
                <td class="text-right"><span class="bold-text dashed-border-bottom text-white">${data.coefficient[type].evebitda !== null && data.coefficient[type].evebitda > 0 ? Intl.NumberFormat(numberLocale).format(data.coefficient[type].evebitda.toFixed(0)) : "N/A"}</td> 
            </tr>`;
    $("#basic-index-evaluation-value").text(`${data.evaluation[type] !== null && data.evaluation[type] > 0 ? Intl.NumberFormat(numberLocale).format(data.evaluation[type].toFixed(0)) : "N/A"}`);
    ...
    if (evaluationsData && evaluationsData.cashFlow && evaluationsData.cashFlow.length > 0) {
        ...
        var res = `<p class="font-weight-bold" style="margin-bottom: 10px !important;">Với giả định như sau: Tăng trưởng <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#fff" style="background: #0d6efd; border-radius: 50%; cursor: pointer;" class="bi bi-info-circle" viewBox="0 0 16 16" data-bs-toggle="tooltip" data-bs-html="true" data-bs-placement="right" title="" data-bs-original-title="<div class='text-left'><p>Giả định này thể hiện tốc độ tăng trưởng trung bình hàng năm khi doanh nghiệp đã đi vào hoạt động ổn định sau kỳ dự báo (hay còn gọi là [Tỉ lệ tăng trưởng trong vô hạn] hay Terminal Growth Rate). Thông thường trong giai đoạn này, công suất sản xuất đã đạt mức tối đa, tăng trưởng của doanh nghiệp chủ yếu đến từ tăng giá bán. Do đó, giả định này thường có giá trị tương đương với lạm phát (khoảng 3%-5%).</p></div>">
                ...
                <tr>
                    ...
                    <td class="text-right pd-evaluation-value"><span class="bold-text dashed-border-bottom">${evaluationsData.cashFlow[0].freeCashFlow !== null ? Intl.NumberFormat(numberLocale).format(enterpriseValue.toFixed(0)) : "N/A"}</span</td>
                </tr>
                <tr>
                    <td class="text-left pd-evaluation"><span class="font-weight-bold">Tiền & tương đương tiền: </span></td>
                    <td class="text-right pd-evaluation-value"><span class="bold-text dashed-border-bottom">${Intl.NumberFormat(numberLocale).format(evaluationsData.cash)}</span</td>
                </tr>
                <tr>
                    <td class="text-left pd-evaluation"><span class="font-weight-bold">Nợ: </span></td>
                    <td class="text-right pd-evaluation-value"><span class="bold-text dashed-border-bottom">${Intl.NumberFormat(numberLocale).format(evaluationsData.shortTermDebt + evaluationsData.longTermDebt)}</span</td>
                </tr>
                <tr>
                    <td class="text-left pd-evaluation"><span class="font-weight-bold">Lợi ích CĐ thiểu số: </span></td>
                    <td class="text-right pd-evaluation-value"><span class="bold-text dashed-border-bottom">${Intl.NumberFormat(numberLocale).format(evaluationsData.minorityInterest)}</span</td>
                </tr>
                <tr>
                    <td class="text-left pd-evaluation"><span class="font-weight-bold">Giá trị vốn hóa DN: </span></td>
                    <td class="text-right pd-evaluation-value"><span class="bold-text dashed-border-bottom">${evaluationsData.cashFlow[0].freeCashFlow !== null && capitalizationValue > 0 ? Intl.NumberFormat(numberLocale).format(capitalizationValue.toFixed(0)) : "N/A"}</span</td>
                </tr>
                <tr>
                    <td class="text-left pd-evaluation"><span class="font-weight-bold">SL cổ phiếu lưu hành: </span></td>
                    <td class="text-right pd-evaluation-value"><span class="bold-text dashed-border-bottom">${Intl.NumberFormat(numberLocale).format(evaluationsData.shareOutstanding)}</span</td>
                </tr>
                  </table>`;
        $("#dcf-evaluation").html(res);
        $("#dcf-evaluation-value").text(`${capitalizationValue > 0 ? Intl.NumberFormat(numberLocale).format((capitalizationValue/evaluationsData.shareOutstanding).toFixed(0)) : "N/A"}`);
    } else {
